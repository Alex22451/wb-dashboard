...
const SUBJECT_TO_EXCEL_TYPES: Array<{ subject: string; types: string[] }> = [
  ...
  { subject: 'Аксессуары для фотосессий', types: ['фотофоны'] },
  { subject: 'Аксессуары для фотосессии', types: ['фотофоны'] },
  { subject: 'Постеры', types: ['фотофоны'] },
    ...
    for (const compositeKey of wbArticleKeys) {
      ...
      if (!matchedProductId) {
        ...
        const KEYWORD_MAP: [string, string][] = [
          ...
          ['ткань', 'ткань'],
          ['набор', 'набор'],
          ['пледдляживот', 'пледы для животных'],
          ['плед', 'плед'],
