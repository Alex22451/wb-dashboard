...
function apiKeyFingerprint(apiKey: string): string {
  return getWbApiKeyFingerprint(apiKey)
}
...
function redisDailyKeyForFingerprint(fingerprint: string, date: string, variant = 'default', metric: DataMetric = 'orders') {
  const variantSuffix = variant === 'default' ? '' : `:${variant}`
  if (metric === 'orders') return `wb:daily:v11:${fingerprint}:${date}${variantSuffix}`
  return `wb:daily:sales:v6:${fingerprint}:${date}${variantSuffix}`
}
...
async function writeRedisDailyPayload(apiKey: string, date: string, daily: any, variant = 'default', metric: DataMetric = 'orders') {
  await redisCommand([
    ...
    JSON.stringify({
      ...
      source: metric === 'orders' ? 'wb-daily-payload-v4' : 'wb-daily-sales-payload-v2',
      metric,
      complete: true,
      daily,
    }),
...
function redisReportKey(
  ...
  return `wb:report:${section}:${metric}:v7:${scopeHash}:${from}:${to}`
}
  ...
  try {
    ...
    if (needDaily || needProduction) {
      const buildDailyPayload = (
        ...
        const dailyRevenueDateTotals: number[] = new Array(visibleDailyDates.length).fill(0)
        ...
        for (const o of mappedRows) {
          ...
          const revenue = getOrderRevenue(o.order)
          dailyRevenueDateTotals[dateIdx] += revenue
        ...
        return {
          ...
          revenueDateTotals: dailyRevenueDateTotals,
          previousDateTotals,
